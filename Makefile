...
.PHONY: all setup clean test lint format run-all run-loglama run-collector run-bexy run-pyllm run-pylama run-apilama run-shellama run-weblama stop-all stop status help

# Default target
all: help

# Setup all projects with virtual environments
setup: setup-loglama setup-bexy setup-pyllm setup-pylama setup-apilama setup-shellama setup-weblama
...
run-all: run-loglama run-collector run-bexy run-pyllm run-shellama run-pylama run-apilama run-weblama
